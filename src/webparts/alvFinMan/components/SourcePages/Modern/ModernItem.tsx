...
export function createModernRow( item: IPagesContent , searchText: string, onClick: any, pageThis: any ) {

    const row = <div className={ styles.modernItem }>
        <div className={ stylesP.itemIcon }><Icon iconName={ SearchTypes.objs[item.typeIdx].icon } onClick = { () => onClick( item.ID, 'pages', item, pageThis ) }></Icon></div>

        <div className={ styles.modernDetails}>
            <div className={ styles.modernRow1 } style={{cursor: item.searchHref ? 'pointer' : null }} onClick = { () => onClick( item.ID, 'pages', item, pageThis ) }>
